...
public class ToppingController {
    ...
    private final IToppingService service;
    ...
    @GetMapping("/list")
    public ResponseEntity<PaginatedResponse<ToppingDTO>> getAllToppings(
             Pageable pageable,
            @RequestParam(required = false) String search,
             @RequestParam(required = false) Boolean isActive

    ) {
        Page<ToppingDTO> dtoPage = service.getAllTopping(pageable, search,isActive);
        PaginatedResponse<ToppingDTO> response = new PaginatedResponse<>(dtoPage, HttpServletResponse.SC_OK, "Lấy danh sách topping thành công");
        return ResponseEntity.ok(response);
    }

    @GetMapping("/customer/list")
    public ResponseEntity<PaginatedResponse<ToppingDTO>> getAllToppingsCustomer(
            @PageableDefault(sort = "id", direction = Sort.Direction.DESC) Pageable pageable,
            @RequestParam(required = false) String search
    ) {
        Page<ToppingDTO> dtoPage = service.getAllToppingCustomer(pageable, search);
        PaginatedResponse<ToppingDTO> response = new PaginatedResponse<>(dtoPage, HttpServletResponse.SC_OK, "Lấy danh sách topping thành công");
        return ResponseEntity.ok(response);
    }
    ...
    @GetMapping("/{id}")
    public ResponseEntity<SuccessResponse<ToppingDTO>> getById(@PathVariable Long id) {
        ToppingDTO dto = service.getById(id);
        SuccessResponse<ToppingDTO> response = new SuccessResponse<>(
                HttpServletResponse.SC_OK,
                "Lấy chi tiết topping thành công!",
                dto
        );
        return ResponseEntity.ok(response);
    }
